...
| mTLS test tool (curl) | `curl` with `--cert` / `--cacert` |
| mTLS test tool (browser) | `./tests/browser_mtls_prep.sh` — generates PKCS#12 and prints import steps |
...
### TC-19 — mTLS: Connection Without Client Certificate (curl)

**Objective:** When mTLS is enabled, a plain curl connection (no client cert) is rejected at TLS handshake.
...
### TC-20 — mTLS: Connection With Valid Client Certificate (curl)
...
### TC-19B — mTLS: Browser Connection Without Client Certificate

**Objective:** When mTLS is enabled, a browser with no client certificate installed is rejected at the TLS handshake level — the page never loads.

| Field | Value |
|-------|-------|
| Prerequisite | Server running with mTLS (Root CA loaded); `browser_mtls_prep.sh` run to pull certs |
| Setup | Run: `./tests/browser_mtls_prep.sh <device-ip>` |
| Action (Firefox) | Open **Private Window** → navigate to `https://<ip>:8443/` |
| Action (Chrome) | Open **Incognito Window** → navigate to `https://<ip>:8443/` |
| Expected (Firefox) | `PR_END_OF_FILE_ERROR` or `SSL_ERROR_HANDSHAKE_FAILURE_ALERT` |
| Expected (Chrome) | `ERR_BAD_SSL_CLIENT_AUTH_CERT` or `ERR_SSL_PROTOCOL_ERROR` |
| Pass/Fail | |
| Notes | Private/Incognito mode prevents auto-selection of installed personal certs in most browsers |

---

### TC-20B — mTLS: Browser Connection With Valid Client Certificate

**Objective:** A browser with the Root CA trusted and the client PKCS#12 cert installed can successfully connect and reach the IoT Gateway login page.

| Field | Value |
|-------|-------|
| Prerequisite | `tests/certs/client.p12` and `tests/certs/rootca.crt` generated by `browser_mtls_prep.sh` |
| Setup step 1 | Import `tests/certs/rootca.crt` as a **trusted CA authority** in the browser |
| Setup step 2 | Import `tests/certs/client.p12` as a **personal certificate** (password: `iotgw`) |
| Action | Navigate to `https://<ip>:8443/` in a normal browser window |
| Expected prompt | Browser shows a "Client Authentication" certificate-selection dialog |
| Expected action | Select **"IoT Gateway Client"** → OK |
| Expected HTTP | `200 OK` — IoT Gateway login/register page loads |
| Pass/Fail | |
| Notes | Firefox: Settings → Privacy & Security → View Certificates. Chrome: Settings → Security → Manage certificates. Full step-by-step printed by `browser_mtls_prep.sh`. |
...
| TC-19 | mTLS no cert (curl) | TLS fail | | |
| TC-20 | mTLS valid cert (curl) | 200 | | |
| TC-19B | mTLS no cert (browser) | TLS error page | | |
| TC-20B | mTLS valid cert (browser) | 200 + page loads | | |
